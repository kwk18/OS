...
#include "zmq.h"
//принимает из стандартного потока ввода строки, а далее отправляет их программе third
...
void SendMessage(void* request, const string& message) {
        zmq_msg_t req;  //массив
        zmq_msg_init_size(&req, message.size());   // инициализация сообщения
        memcpy(zmq_msg_data(&req), message.c_str(), message.size()); // c_str -> req
...
string ReceiveMessage(void* respond) {
        string result = "";
        zmq_msg_t reply;
        zmq_msg_init(&reply); //инициализация пустого сообщ перед отправкой
...
int main() {
        ...
        for(;;) {
                ...
                if (received) {//До тех пор пока программа не примет «сообщение о получение строки» от программы third, она не может отправлять следующую строку программе third.
                        SendMessage(request_c, strings[pos]);
