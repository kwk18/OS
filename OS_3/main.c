...
int MAX = 256;// Колличество эллементов в массиве
int THREAD_MAX;//колличество потоков

int part=0;//переменная, которая будет хранить колличество потоков
int* a ;//указатель на массив для храниения эллементов



// функцию для слияния двух частей
...
int n1 = mid - low + 1, nr = high - mid, i, j;
    ...
    // обьеденяем левую и правую половины в порядке возрастания
    while(i < n1 && j < nr)
    {
        if(left[i] <= right[j])
            a[k++] = left[i++];
...
void marge_rec(int tread_m)//реккурчивная функция сбора данных после отработки потокове
{
...
//находим максимальное колличество потоков
int pow_ = 0;
while (1)
{
int p = MAX >> pow_;
...
THREAD_MAX = MAX/16;
    ...
    t1 = clock();//запоминаем время начала
    ...
    for(int i = 0; i < THREAD_MAX; i++)
     ...
     //объеденяем результаты работы потоков
marge_rec(2);
...
printf("Колличество потоков: %d",THREAD_MAX);
    return 0;
}
